...
function contractAddress() {
  ...
  if (!/^0x[0-9a-fA-F]{40}$/.test(addr)) {
    throw new Error("[FootMon] NEXT_PUBLIC_CONTRACT_ADDRESS is not set to a valid address");
  }
  return addr;
}

export function isChainConfigured() {
  return /^0x[0-9a-fA-F]{40}$/.test(process.env.NEXT_PUBLIC_CONTRACT_ADDRESS || "");
}
