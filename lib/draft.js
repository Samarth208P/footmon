...
/** Default formation (backwards-compatible with pre-choice duels). */
...
/**
 * Formation → slot position list. Must stay in sync with FORMATIONS in
 * lib/constants.js. Extracted here so the server never imports the bulky
 * client-side constants file.
 */
export const FORMATION_SLOTS = {
  "4-3-3": ["GK", "LB", "CB", "CB", "RB", "CM", "CM", "CM", "LW", "ST", "RW"],
  "4-4-2": ["GK", "LB", "CB", "CB", "RB", "LM", "CM", "CM", "RM", "ST", "ST"],
  "4-2-3-1": ["GK", "LB", "CB", "CB", "RB", "CM", "CM", "LM", "CM", "RM", "ST"],
...
/** Formations available for duel selection. */
export const DUEL_FORMATIONS = Object.keys(FORMATION_SLOTS);

/**
 * Get the slot positions for a given formation key.
 * Falls back to DUEL_SLOTS (4-3-3) for unknown formations.
 */
export function getFormationSlots(formationKey) {
  return FORMATION_SLOTS[formationKey] || DUEL_SLOTS;
}
...
/**
 * Get the position for a slot index within a given formation.
 * @param {number} slotIndex
 * @param {string} [formationKey] - Formation key (defaults to DUEL_FORMATION)
 */
export function slotPositionFor(slotIndex, formationKey) {
  const slots = formationKey ? getFormationSlots(formationKey) : DUEL_SLOTS;
 ...
 * @param {number} slotIndex
 * @param {string|string[]} playerPositions
 * @param {string} [formationKey] - Formation key (defaults to DUEL_FORMATION)
 */
export function canFillSlot(slotIndex, playerPositions, formationKey) {
  const slotPos = slotPositionFor(slotIndex, formationKey);
 ...
 * @param {string} [p.formation]        formation key for slot position lookup
...
export function validatePick({
  ...
  if (!canFillSlot(slotIndex, playerPositions, formation)) {
    return {
      ok: false,
      status: 400,
      error: `A ${Array.isArray(playerPositions) ? playerPositions.join("/") : playerPositions} cannot play ${slotPositionFor(slotIndex, formation)}`,
    };
  }
