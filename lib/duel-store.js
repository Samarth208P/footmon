...
function unwrap(result, context) {
  ...
  if (error) {
    ...
    const detail =
      permissionDenied && isPublishableKeyInSecretSlot()
        ? `${error.message} — ${PUBLISHABLE_KEY_WRITE_ERROR}`
        : error.message;
...
export async function pickSlot({
  ...
  if (!usingSupabase()) {
    ...
    const slot = { id: nextId(), ...row, picked_at: nowIso() };
    memory.slots.push(slot);
    return slot;
  }

  return unwrap(
    await getServerClient().from("duel_squad_slots").insert(row).select().single(),
    "pickSlot"
  );
}
