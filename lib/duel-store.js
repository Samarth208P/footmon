...
export async function listDuelLeaderboard(limit = 50) {
  ...
  const aggregateRooms = (rooms, profiles) => {
    const stats = new Map();
    const getOrCreate = (address) => {
      const addrKey = addr(address);
      if (!stats.has(addrKey)) {
        stats.set(addrKey, {
          address: addrKey,
          wins: 0,
          losses: 0,
          ...
          goals_against: 0,
          mon_won: 0n,
          username: profiles.find(p => addr(p.address) === addrKey)?.username || "unclaimed"
        });
      }
      return stats.get(addrKey);
    };

    for (const room of rooms) {
      if (!room.creator || !room.joiner) continue;
      const creator = addr(room.creator);
      const joiner = addr(room.joiner);
      const winner = room.winner ? addr(room.winner) : null;
      const isDraw = room.is_draw;

      const creatorStats = getOrCreate(creator);
      const joinerStats = getOrCreate(joiner);

      const creatorGoals = Number(room.score_creator ?? 0);
      const joinerGoals = Number(room.score_joiner ?? 0);

      creatorStats.goals_for += creatorGoals;
      creatorStats.goals_against += joinerGoals;
      joinerStats.goals_for += joinerGoals;
      joinerStats.goals_against += creatorGoals;

      let payout = 0n;
      if (room.resolver_tx && !isDraw && winner) {
        const stakeBig = BigInt(room.stake || "0");
        payout = (stakeBig * 2n * 70n) / 100n;
      }
      ...
      if (isDraw) {
        ...
        creatorStats.wins++;
        joinerStats.losses++;
        creatorStats.mon_won += payout;
      } else if (winner === joiner) {
        joinerStats.wins++;
        creatorStats.losses++;
        joinerStats.mon_won += payout;
      }
    }

    const list = Array.from(stats.values()).map(item => ({
      ...item,
      mon_won: item.mon_won.toString(),
  ...
  if (!usingSupabase()) {
    ...
    const todayRooms = memory.rooms.filter(
      (r) => r.status === "complete" && r.updated_at >= startUtc && r.updated_at <= endUtc
    );
    return aggregateRooms(todayRooms, memory.profiles).slice(0, limit);
  }

  const rooms = unwrap(
    await getServerClient()
      .from("duel_rooms")
      .select("creator, joiner, winner, is_draw, score_creator, score_joiner, stake, resolver_tx, resolved_at, updated_at")
      .eq("status", "complete")
      .gte("updated_at", startUtc)
      .lte("updated_at", endUtc),
    "listDuelLeaderboard:rooms"
  ) || [];

  const addresses = new Set();
  for (const r of rooms) {
    if (r.creator) addresses.add(addr(r.creator));
    if (r.joiner) addresses.add(addr(r.joiner));
  }
  ...
  if (addressList.length > 0) {
    profiles = unwrap(
      ...
      "listDuelLeaderboard:profiles"
    ) || [];
  }

  return aggregateRooms(rooms, profiles).slice(0, limit);
}
...
export async function listTournamentLeaderboard(limit = 50) {
  ...
  if (!usingSupabase()) {
    ...
    const todayRuns = memory.tournamentRuns.filter(
      (run) => run.completed_at >= startUtc && run.completed_at <= endUtc
    );
    const joined = todayRuns.map(run => {
      const p = memory.profiles.find(x => addr(x.address) === addr(run.address));
      return {
        ...run,
        username: p ? p.username : "unclaimed"
      };
    });
    joined.sort(
      (a, b) =>
    ...
    return joined.map((run, idx) => ({ ...run, rank: idx + 1 })).slice(0, limit);
  }

  const runs = unwrap(
    await getServerClient()
      .from("tournament_leaderboard")
      .select("address, wins, goals_for, goals_against, goal_diff, team_rating, nation, year, formation, completed_at, profiles:address(username)")
      .gte("completed_at", startUtc)
      .lte("completed_at", endUtc),
    "listTournamentLeaderboard:raw"
  ) || [];
  ...
  const joined = runs.map(run => ({
    ...
    goal_diff: run.goal_diff,
    team_rating: run.team_rating,
    nation: run.nation,
    year: run.year,
    formation: run.formation,
    completed_at: run.completed_at,
    username: run.profiles?.username || "unclaimed"
  }));
