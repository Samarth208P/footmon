 ...
 *
 * At parity (equal ratings, equal chemistry) both sides expect ~1.15 goals,
 * which yields realistic ~35 / 30 / 35 outcome splits over a large sample.
 * A 10-point rating gap moves the favourite's xG to ~1.9 vs ~0.6, giving
 * roughly a 75-80% win rate. A 20-point gap is near-decisive (~92% win).
 *
 * @param {{attack: number, defence: number, midfield: number}} own
 * @param {{attack: number, defence: number, midfield: number}} opp
 * @param {{isHome?: boolean, neutralVenue?: boolean}} [opts]
 */
function expectedGoals(own, opp) {
  const attackEdge = own.attack - opp.defence;
  const midfieldEdge = own.midfield - opp.midfield;

  // Sensitivity constants. Attacking edge dominates; midfield adds a smaller
  // tempo advantage that mostly matters when the attack/defence gap is small.
  const ATTACK_K = 0.055;
  const MIDFIELD_K = 0.020;
  // Base xG per side at parity. Tuned to land total goals-per-match around
  // 2.8–3.0, slightly above the real-football average of ~2.6 — the game
  // feels more alive with a few more chances converted.
  const BASE = 1.50;
 ...
 * plus a midfield tempo edge. Hidden chemistry — same-nation and same-year
 * cores plus playing in a natural position — silently boosts both attack
 * and defence, so a well-connected squad plays above its raw rating.
 *
 * Same seed + same squads => identical scoreline, minutes, and scorers.
 *
 * The engine is deliberately mode-agnostic: tournament and duel matches
 * follow the exact same rules. The only knob a caller flips is `knockout`,
 * which decides whether a level score at 90' goes to penalties or stays a
 * draw. There is no "home advantage" — both sides play on neutral ground.
 *
 * @param {object} opts
 ...
 * }}
 */
export function simulateMatch({ seed, home, away, knockout = false }) {
  ...
  const homeBoost = chemistryBoost(homeChem);
  const awayBoost = chemistryBoost(awayChem);
  ...
  // Both sides play on neutral ground — no home boost. Duel and tournament
  // matches produce identical distributions for identical inputs.
  const homeXg = expectedGoals(homeStrength, awayStrength);
  const awayXg = expectedGoals(awayStrength, homeStrength);
