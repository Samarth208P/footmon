...
// ── Formation Matchup System ────────────────────────────────────────────────
//
// Formations interact tactically like rock-paper-scissors, rewarding players
// who pick the right shape vs their opponent's. Each formation has inherent
// strengths that translate into attack/defence/midfield modifiers.
//
// The matchup matrix is inspired by real football tactics:
//   - Wide formations (4-3-3, 3-4-3) stretch narrow formations (4-4-2, 4-5-1)
//   - Compact midfields (4-5-1, 3-5-2) control the center vs spread formations
//   - Defensive depth (5-3-2) frustrates attacking formations but struggles to score
//   - Extra strikers (4-2-4) overwhelm defensive lines but leave gaps in midfield
//
// Modifiers are small (+/- 1.5 to 3.0) — enough to matter when squads are close
// in rating, but a strong squad still beats a weaker one in the wrong formation.

/**
 * Formation attributes: each formation has base tactical properties.
 * Values are multipliers applied to the corresponding strength axis.
 *
 *   attack:   how many forward threats the formation creates (more STs/wingers = higher)
 *   defence:  how solid the back line is (more CBs/defensive shape = higher)
 *   midfield: how much the formation controls the centre (more CMs = higher)
 *   width:    how well the formation uses wide areas (wingers/fullbacks pushing up)
 */
const FORMATION_ATTRIBUTES = {
  "4-3-3":   { attack: 1.05, defence: 0.95, midfield: 1.00, width: 1.10 },
  "4-4-2":   { attack: 1.00, defence: 1.00, midfield: 1.05, width: 0.95 },
  "4-2-3-1": { attack: 0.95, defence: 1.05, midfield: 1.10, width: 0.90 },
  "4-2-4":   { attack: 1.15, defence: 0.85, midfield: 0.85, width: 1.05 },
  "3-5-2":   { attack: 0.95, defence: 0.95, midfield: 1.15, width: 1.00 },
  "5-3-2":   { attack: 0.85, defence: 1.15, midfield: 0.95, width: 0.80 },
  "4-5-1":   { attack: 0.80, defence: 1.10, midfield: 1.15, width: 0.95 },
  "3-4-3":   { attack: 1.10, defence: 0.85, midfield: 0.95, width: 1.15 },
};

/**
 * Directional matchup edges: how well formation A performs AGAINST formation B.
 * Stored as [attacker, defender] → { attackMod, defenceMod, midfieldMod }.
 * Missing pairs default to neutral (0).
 *
 * Only asymmetric advantages are stored — if A has an edge attacking B,
 * B doesn't automatically have an edge attacking A.
 */
const FORMATION_MATCHUPS = {
  // Wide formations stretch narrow ones
  "4-3-3 vs 4-4-2":   { attackMod: 1.5,  defenceMod: -0.5, midfieldMod: -1.0 },
  "4-3-3 vs 4-5-1":   { attackMod: 2.0,  defenceMod: -1.0, midfieldMod: -1.5 },
  "3-4-3 vs 4-4-2":   { attackMod: 2.0,  defenceMod: -1.5, midfieldMod: -0.5 },
  "3-4-3 vs 4-5-1":   { attackMod: 2.5,  defenceMod: -1.0, midfieldMod: -1.0 },

  // Midfield dominance chokes forward-heavy formations
  "4-5-1 vs 4-2-4":   { attackMod: 0.5,  defenceMod: 2.0,  midfieldMod: 2.5 },
  "4-5-1 vs 3-4-3":   { attackMod: -0.5, defenceMod: 2.0,  midfieldMod: 1.5 },
  "3-5-2 vs 4-2-4":   { attackMod: 1.0,  defenceMod: 1.5,  midfieldMod: 2.0 },
  "3-5-2 vs 4-3-3":   { attackMod: 0.0,  defenceMod: 0.5,  midfieldMod: 1.5 },

  // Defensive depth frustrates attackers
  "5-3-2 vs 4-3-3":   { attackMod: -1.0, defenceMod: 2.5,  midfieldMod: -0.5 },
  "5-3-2 vs 4-2-4":   { attackMod: -0.5, defenceMod: 3.0,  midfieldMod: 0.0 },
  "5-3-2 vs 3-4-3":   { attackMod: -1.0, defenceMod: 2.5,  midfieldMod: 0.0 },

  // High-pressing forward formations overwhelm low blocks
  "4-2-4 vs 5-3-2":   { attackMod: 2.5,  defenceMod: -2.0, midfieldMod: -1.0 },
  "4-2-4 vs 4-5-1":   { attackMod: 2.0,  defenceMod: -1.5, midfieldMod: -2.0 },

  // Balanced formations have mild edges
  "4-4-2 vs 3-5-2":   { attackMod: 0.5,  defenceMod: 0.5,  midfieldMod: -1.0 },
  "4-4-2 vs 5-3-2":   { attackMod: 1.0,  defenceMod: 0.0,  midfieldMod: 0.5 },
  "4-2-3-1 vs 4-4-2": { attackMod: 0.5,  defenceMod: 0.5,  midfieldMod: 1.0 },
  "4-2-3-1 vs 3-4-3": { attackMod: 0.0,  defenceMod: 1.5,  midfieldMod: 1.0 },
};

/**
 * Calculates formation-based tactical modifiers for a side.
 *
 * @param {string|null} ownFormation  - The attacking side's formation (e.g. "4-3-3")
 * @param {string|null} oppFormation  - The defending side's formation
 * @returns {{ attack: number, defence: number, midfield: number }}
 *   Additive modifiers to apply to the side's strength ratings.
 */
export function formationMatchupBonus(ownFormation, oppFormation) {
  const neutral = { attack: 0, defence: 0, midfield: 0 };
  if (!ownFormation || !oppFormation) return neutral;

  // Base attributes: the formation's inherent shape gives a passive boost.
  // A 4-2-4 is just naturally more attacking regardless of opponent.
  const ownAttrs = FORMATION_ATTRIBUTES[ownFormation];
  if (!ownAttrs) return neutral;

  // Passive: formation's own shape modifier (deviation from 1.0, scaled to rating points).
  // A formation with attack=1.15 gives +1.5 attack; one with 0.85 gives -1.5.
  const passiveScale = 10; // ±0.15 * 10 = ±1.5 rating points
  let attackBonus = (ownAttrs.attack - 1.0) * passiveScale;
  let defenceBonus = (ownAttrs.defence - 1.0) * passiveScale;
  let midfieldBonus = (ownAttrs.midfield - 1.0) * passiveScale;

  // Directional matchup: specific formation vs formation edges.
  const matchupKey = `${ownFormation} vs ${oppFormation}`;
  const matchup = FORMATION_MATCHUPS[matchupKey];
  if (matchup) {
    attackBonus += matchup.attackMod;
    defenceBonus += matchup.defenceMod;
    midfieldBonus += matchup.midfieldMod;
  }

  return {
    attack: attackBonus,
    defence: defenceBonus,
    midfield: midfieldBonus,
  };
}
...
export function simulateMatch({ seed, home, away, knockout = false, mode = "duel" }) {
  ...
  // Formation tactical modifiers: each side's shape creates matchup edges
  // (rock-paper-scissors style). Only active in duel mode — solo uses
  // neutral AI opponents with no formation choice.
  const formationOn = mode !== "solo";
  const homeFormationMod = formationOn
    ? formationMatchupBonus(home.formation, away.formation)
    : { attack: 0, defence: 0, midfield: 0 };
  const awayFormationMod = formationOn
    ? formationMatchupBonus(away.formation, home.formation)
    : { attack: 0, defence: 0, midfield: 0 };

  const homeStrength = {
    attack:   attackRating(home.players)   + homeBoost + homeFormationMod.attack,
    defence:  defenceRating(home.players)  + homeBoost + homeFormationMod.defence,
    midfield: midfieldRating(home.players) + homeBoost * 0.5 + homeFormationMod.midfield,
  };
  const awayStrength = {
    attack:   attackRating(away.players)   + awayBoost + awayFormationMod.attack,
    defence:  defenceRating(away.players)  + awayBoost + awayFormationMod.defence,
    midfield: midfieldRating(away.players) + awayBoost * 0.5 + awayFormationMod.midfield,
  };
 ...
 *
 * Opponents use a FIXED rating ladder — the same for every player. A
 * stronger squad has a genuine statistical edge: a 94-rated team will
 * dominate the final (~89 AI) while an 82-rated team barely survives R16.
 *
 *   round 1  → ~72  (group minnow)
 *   round 4  → ~82  (competitive R16)
 *   round 7  → ~89  (elite final)
 */
export function buildTournamentLadder({ seed, playerRating, rounds = 7 }) {
  const rng = createRng(`${seed}:ladder`);

  // Fixed opponent ratings — the same for every player, every run. A
  // stronger squad genuinely wins more often. The ramp mirrors a real
  // World Cup: weak group minnows → competitive knockouts → elite final.
  const FIXED_RATINGS = [72, 75, 78, 82, 85, 87, 89];

  const ladder = [];
  for (let round = 1; round <= rounds; round++) {
    const baseRating = FIXED_RATINGS[round - 1] ?? 89;
    // Small jitter (±1.5) so consecutive runs feel slightly different,
    // but the overall difficulty band stays consistent.
    const jitter = (rng() - 0.5) * 3;
    const rating = clamp(baseRating + jitter, 40, 99);
