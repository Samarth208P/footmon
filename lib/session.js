...
/** Wallet-level sessions (e.g. for rerolls) */

export function buildWalletSessionMessage({ address, issuedAt, nonce }) {
  return [
    "FootMon wallet session",
    "",
    `Address: ${normaliseAddress(address)}`,
    `Issued At: ${issuedAt}`,
    `Nonce: ${nonce}`,
    "",
    "Signing authorises this browser to use your reroll credits during this session.",
    "It costs no gas and sends no transaction.",
  ].join("\n");
}

export function createWalletSessionToken({ address, ttlMs = SESSION_TTL_MS }) {
  const payload = {
    a: normaliseAddress(address),
    e: Date.now() + ttlMs,
  };
...
export function verifyWalletSessionToken(token) {
  if (typeof token !== "string" || !token.includes(".")) return null;
