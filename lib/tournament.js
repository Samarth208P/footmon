...
export function runTournament({ seed, players, ladder: prebuiltLadder }) {
  ...
  for (const rung of ladder) {
    ...
    // Knockout tournament: a draw goes to extra time → penalties.
    // We award the win to the player (home) on a draw — simulating
    // a narrow penalty shoot-out advantage to keep the run alive.
    const won = match.winner === "home" || match.winner === null;
